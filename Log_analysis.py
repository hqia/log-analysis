...
def str2bin(data_str):
    data = bin(int(data_str,16)).split("0b")[1].zfill(16)
    return data 

def show_AGain(again):
    # 4通道模拟增益显示值
    FL_AGain = again_tab.get(str2bin(again)[13:])
    FR_AGain = again_tab.get(str2bin(again)[9:12])
    RL_AGain = again_tab.get(str2bin(again)[5:8])
    RR_AGain = again_tab.get(str2bin(again)[1:4])
    print("Analog Gain")
